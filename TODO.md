...
## VID (Manim) Porting Gaps

The `animations/VID/**/*.cpp` files are autogenerated Catalyst translation scaffolds placed next to the original Manim sources.
Porting these fully will require adding or emulating several Manim engine concepts:

- [x] Updaters: `add_updater`, `always_redraw`, `ValueTracker` (per-frame callbacks / reactive redraw)
- [x] Vector-mobject path API (`VMobject`): bezier editing, `set_points_smoothly`, partial curves, etc.
- [x] 2D opacity controls for fill/stroke/text/math/image elements
- [x] Submobject indexing/slicing + per-substring coloring (`t2c`, `set_color_by_tex`, `VGroup[...]`)
- [x] Spatial helpers: `shift`, `move_to` (instant), `next_to`, `to_edge/to_corner`, `align_to`
- [x] SVG import (`SVGMobject`) and richer vector shape ops
- [x] 3D camera “frame” parity + `fix_in_frame` overlays for 3D scenes
- [x] NumPy/SciPy-style utilities used throughout VID (ODE solvers, interpolation, color gradients)
